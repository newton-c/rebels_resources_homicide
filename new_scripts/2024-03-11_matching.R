...
datasets <- map(seq_len(5), function(i) {
    #file_path <- paste0("data/imputed_data_2_", i, ".csv")
    #datasets[[i]] <- read_csv(file_path)
    datasets[[i]] <- a.out$imputations[[i]]
})

#datasets[[6]] <- read_csv("data/unimputed_data2.csv") %>%
#    drop_na()

#match_formula <- illicit_resources ~ pop_slums + gini + corruption +
#                 pko_troops + pko_police + total_deaths + log_pop

match_formula <- resources ~ pop_slums + battle_deaths + log_pop

match_algos <- list("cem", "nearest", "optimal")
...
percent_change <- glm.nb(hom_count ~ illicit_resources,
                          data = match.data(matched_data[[9]]),
                          weights = match.data(matched_data[[9]])$weights)

predict(percent_change, data.frame(illicit_resources = 1), type = "response")
predict(percent_change, data.frame(illicit_resources = 0), type = "response")
